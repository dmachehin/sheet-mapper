...
- `#[SheetMapping]` на классе описывает лист и наличие строки заголовков. Если `target_sheet` не указан, при чтении файла будут обработаны все листы книги.
- `#[SheetField]` на свойствах задаёт номер колонки (с нуля) либо имя заголовка.
- Для merge-ячеек можно точечно включить наследование значения из master-ячейки через `allow_merge`.
